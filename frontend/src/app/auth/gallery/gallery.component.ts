...
export class GalleryComponent implements OnInit {
  ...
  public base64Encodings: any[]; //array cu codarile in base64 ale imaginilor
    ...
    this.imagesService.getImages().subscribe(
      images => {
        ...
        for (let image of images) {
          console.log(image);
          this.base64Encodings.push(image.base64String);
  ...
  private getAllImagesByEventId(selectedEventId) {
    if (selectedEventId === 'All Events')
      ...
      this.imagesService.getImagesByEventId(selectedEventId).subscribe( res =>{
        res.forEach((doc) =>{
          this.base64Encodings.push(doc.base64String);
        })
      }
      )
